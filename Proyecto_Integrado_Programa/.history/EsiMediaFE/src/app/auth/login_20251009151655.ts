...
export class LoginComponent implements OnDestroy {
  ...
  // Usuario logueado con nombre, email y rol opcional
  user: { nombre: string; email: string; rol?: string } | null = null;
  ...
  submit() {
    ...
    this.auth.login(this.form.value as any).subscribe({
      next: (res: BackendLoginResponse) => {
        ...
        if (res.user) {
          // Asignación segura del usuario
          this.user = {
            nombre: res.user.nombre ?? res.user.email,
            email: res.user.email,
            rol: res.user.rol ?? undefined
          };
          this.okMsg = `Bienvenido, ${this.user.nombre}`;
