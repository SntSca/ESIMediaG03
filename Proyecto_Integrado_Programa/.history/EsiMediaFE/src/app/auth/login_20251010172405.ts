...
export class LoginComponent implements OnDestroy {
  ...
  /** 👇 helper centralizado para redirigir según el role del usuario */
    ...
    this.auth.login(this.form.value as any).subscribe({
      next: (res: BackendLoginResponse) => {
        ...
        if (res.user) {
          ...
          this.step = 'done';
          this.redirectByRole(res.user);   // 👈 redirección por rol
          return;
        }
    ...
    this.auth.verifyMfa({ mfaToken: this.mfaToken, code }).subscribe({
      next: (r: BackendLoginResponse) => {
        ...
        if (r?.user) {
          ...
          this.step = 'done';
          this.redirectByRole(r.user);     // 👈 redirección por rol
          return;
        }
