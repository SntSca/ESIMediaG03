...
export class LoginComponent implements OnDestroy {
  ...
  /** Navegación con View Transitions y paso de usuario por state */
  private navigateWithTransition(url: string, user: UserDto) {
    const go = () =>
      this.router.navigateByUrl(url, {
        replaceUrl: true,
        state: { user }, // 👈 aquí viaja el usuario
      });
  ...
  /** Redirige según rol y aplica transición pasando el usuario */
    ...
    this.auth.login(this.form.value as any).subscribe({
      next: (res: BackendLoginResponse) => {
        ...
        if (res.user) {
          ...
          this.auth.saveSession(res.user);   // persistencia
          this.currentUser = res.user;
          this.step = 'done';
          this.redirectByRole(res.user);     // transición + paso de user
          return;
        }
    ...
    this.auth.verifyMfa({ mfaToken: this.mfaToken, code }).subscribe({
      next: (r: BackendLoginResponse) => {
        ...
        if (r?.user) {
          ...
          this.step = 'done';
          this.redirectByRole(r.user); // transición + paso de user
          return;
        }
