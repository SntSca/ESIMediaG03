...
export class LoginComponent implements OnDestroy {
    ...
    this.auth.verifyMfa({ mfaToken: this.mfaToken, code }).subscribe({
      next: (r: any) => {
        ...
        if (r?.ok === true || r?.status === 'OK') {
          this.finalizeLogin(); // obtiene /me si no viene user
          return;
        }
  ...
  // =========================
  // Routing por rol y finalización
  // =========================

  private navigateByRole(role: string | undefined | null) {
    const r = (role ?? '').toUpperCase();
    if (r === 'ADMINISTRADOR') this.router.navigate(['/admin']);
    else if (r === 'GESTOR DE CONTENIDO') this.router.navigate(['/gestor']);
    else if (r === 'USUARIO') this.router.navigate(['/usuario']);
    else this.router.navigate(['/auth/login']); // fallback
  }
  ...
  private finalizeLogin(userMaybe?: Partial<UserDto>) {
    ...
    this.auth.me().subscribe({
      next: (u) => { this.loading = false; proceed(u); },
      error: () => {
        this.loading = false;
