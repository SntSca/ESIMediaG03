...
export class LoginComponent implements OnDestroy {
  ...
  submit() {
    ...
    this.auth.login(this.form.value).subscribe({
      next: (res: BackendLoginResponse) => {
        ...
        this.errorMsg = 'Problema al intentar acceder a la plataforma. Contacta con un administrador.';
      },
    ...
    this.auth.verifyCaptcha({ captchaToken: this.captchaToken, answer }).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        ...
        if (body?.needMfa3 && body?.captchaToken && body?.captchaImage) {
          ...
          this.captchaForm.reset();
          this.errorMsg = 'Respuesta incorrecta. Intenta con el nuevo captcha.';
          return;
        }
  ...
  private handleFriendlyError(err: HttpErrorResponse) {
    ...
    if (data.message) this.errorMsg = data.message;
    this.remainingAttempts = Number.isFinite(data.attemptsLeft as number) ? data.attemptsLeft! : null;

    if (Number.isFinite(data.retryAfterSec as number) && (data.retryAfterSec as number) > 0) {
      this.startCountdown(data.retryAfterSec!);
    }
  }
