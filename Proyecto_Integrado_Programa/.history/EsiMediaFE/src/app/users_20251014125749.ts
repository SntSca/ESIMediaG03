...
export interface RegistroDatos {
  ...
  pwd2: string;
  vip?: boolean;
  foto: string;              // obligatorio (ruta/url/base64 en tu caso)
  role: RoleUI;              // valores del selector del FE
...
export class UsersService {
  ...
  /** Normaliza el rol del FE al esperado por el backend */
  ...
  /**
   * Registrar usuario
   * - alias y foto son obligatorios
   * - si role === Gestor de Contenido => descripcion, especialidad y tipoContenido (Audio/Video) obligatorios
   */
  registrar(datos: RegistroDatos): Observable<string> {
    ...
    // Adjuntar campos del Gestor si aplica
    if (datos.role === 'Gestor de Contenido') {
      payload.descripcion = (datos.descripcion ?? '').trim();
      payload.especialidad = (datos.especialidad ?? '').trim();
      // Backend espera AUDIO/VIDEO en mayúsculas; mandamos exactamente eso
      payload.tipoContenido = (datos.tipoContenido ?? '').toUpperCase(); // 'AUDIO' | 'VIDEO'
    }

    // Devuelve texto plano (200 OK sin body JSON)
