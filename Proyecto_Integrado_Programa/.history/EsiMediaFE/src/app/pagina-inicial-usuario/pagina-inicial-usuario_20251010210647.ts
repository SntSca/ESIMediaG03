...
export class PaginaInicialUsuario implements OnInit {
  ...
  // ===== MOCK con URLs reales de prueba =====
  cargarMock(): void {
    const now = new Date();
    const isoAgo = (h: number) => new Date(now.getTime() - h * 3600_000).toISOString();

    // Videos de muestra libres: Google sample videos (Big Buck Bunny, Sintel, Tears of Steel)
    // Audios de muestra libres: SoundHelix
  ...
  // === Play: abre la URL del contenido ===
  ...
  toggleFavorito(contenido: ContenidoFront, ev?: Event): void {
    ...
    const req$ = estabaFavorito
      ? this.contenidosService.removeFavorito(userId, contenido.id)
