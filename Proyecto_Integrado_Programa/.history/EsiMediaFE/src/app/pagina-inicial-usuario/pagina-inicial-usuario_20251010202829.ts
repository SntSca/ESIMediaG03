...
toggleFavorito(contenido: ContenidoFront): void {
  ...
  // 1) Id de usuario (adapta si tu UserDto usa otra propiedad)
  ...
  // 2) Lock por (userId, contenidoId) para evitar dobles clics
  ...
  // 4) Llamada al backend con userId + contenidoId (ambos string)
  this.contenidosService
    .setFavorito(userId, contenido.id, nuevoEstado)
    .pipe(finalize(() => this.favLocks.delete(lockKey)))
    .subscribe({
      next: () => {
        // ok, nada más (optimistic UI ya aplicado)
      },
      error: (err) => {
