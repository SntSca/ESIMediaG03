...
export class PaginaInicialUsuario implements OnInit {
  ...
  // 👉 1) Estructura para cada rail-tema
interface RailSection {
  key: string;            // clave normalizada, p. ej. "drama"
  title: string;          // título visible, p. ej. "Drama"
  items: ContenidoFront[]; // contenidos del tema
}

// 👉 2) Config rápida
private readonly RAILS_MAX = 12;        // máx. nº de railes a mostrar
private readonly RAIL_ITEMS_MAX = 20;   // máx. ítems por rail
private readonly MIN_ITEMS_PER_RAIL = 2;// mínimo para generar un rail
private readonly MULTI_ASSIGN = true;   // si true, un contenido entra en todos sus tags

// 👉 3) Normalizadores/ayudas para temas
...
// 👉 4) Fuente de datos base = conjunto activo por pestaña/subpestaña
//     (ya tienes get contenidosTab() definido en tu TS actual)

// 👉 5) Generar railes por tema a partir del conjunto activo
get railSections(): RailSection[] {
  const src = this.contenidosTab; // usa tu getter existente (respeta pestañas)
  ...
  for (const c of src) {
    ...
    if (this.MULTI_ASSIGN) {
      ...
      // asignación simple: primer tag como “tema principal”
  ...
  // Construye las secciones, aplica mínimos y límites
  ...
  // Ordena railes por tamaño descendente (temas con más contenidos primero)
