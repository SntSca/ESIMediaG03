...
export class PaginaInicialAdmin implements OnInit {
  ...
  ngOnInit(): void {
    // 1) Intenta recuperar desde router state (si vienes del login con navigate state)
    const stateUser = (history.state?.user ?? null) as UserDto | null;

    // 2) Fallback: si actualizas o entras directo, tira de sesión
    const sessionUser = this.auth.getCurrentUser?.() ?? this.getUserFromLocalStorage();

    this.setLoggedUser(stateUser ?? sessionUser ?? null);

    // 3) Carga de usuarios (luego excluimos al logueado)
    this.cargarUsuarios();
  }

  /** Establece el usuario logueado y rellena los campos del header */
  private setLoggedUser(user: UserDto | null) {
    this.loggedUser = user;

    if (!user) return; // sin usuario -> header queda con los valores por defecto

    const nombre = user.nombre?.trim() || user.email.split('@')[0];
    this.userName = nombre;
    this.userEmail = user.email;
    this.userRole = this.mapRoleToLabel(user.role); // 'Administrador' / 'Usuario' / 'Gestor de contenido'
    this.userInitials = this.getInitials(this.userName);

    // Foto si la trajeras en user (extensible)
    // @ts-ignore - por si tu back añade campo 'foto' no tipado
    const foto = (user as any)?.foto as string | null | undefined;
    if (foto) this.userFotoUrl = this.getPhotoUrl(foto);
  }

  /** Mapea el role técnico a una etiqueta amigable */
  ...
  /** Recupera user de localStorage si no tienes getCurrentUser en el servicio */
  ...
  /** 🔹 Carga los usuarios desde el backend y excluye al logueado */
  cargarUsuarios(): void {
    this.loading = true;
    this.errorMsg = '';
    ...
    this.usersService.listarUsuarios().subscribe({
      next: (data) => {
        ...
        // 2) Excluye al usuario logueado por id o, si no hay id, por email
  ...
  /** Convierte ruta relativa a absoluta; respeta data: y URLs absolutas */
  ...
  /** Si la imagen falla, oculta <img> y muestra initials-holder (si lo tienes en la vista) */
  ...
  /** Getter de conveniencia para la tabla */
  ...
  cerrarSesion(): void {
    ...
    if (confirmacion) {
      this.auth.logout?.(); // si tu servicio lo tiene
