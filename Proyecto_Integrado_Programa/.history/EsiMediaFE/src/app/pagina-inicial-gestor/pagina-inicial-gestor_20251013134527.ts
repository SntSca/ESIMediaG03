...
export class PaginaInicialGestor implements OnInit {
  ...
  nuevo: {
    ...
    portada: string; // 👈 ruta de imagen (texto)
  } = {
    titulo: '',
    descripcion: '',
    tipo: '',
  ...
  async onSubmit(form: NgForm) {
    ...
    // Construcción del payload normalizado (JSON)
    const payload: ContenidoCreate = {
      ...
      // 👇 Solo guardamos la ruta (si viene vacía, va como null/undefined)
      portada: this.nuevo.portada?.trim() ? this.nuevo.portada.trim() : null
    };

    this.loading = true;
    this.lastSubmitAt = now;

    // Enviamos SIEMPRE JSON (nada de FormData)
