...
export class PaginaInicialGestor implements OnInit {
  ...
  subirContenido(form: NgForm): void {
    this.errorMsg = '';
    this.successMsg = '';
    ...
    if (this.crossErrorsPresent) {
      ...
      this.errorMsg = 'Revisa los campos: hay incompatibilidades con el tipo seleccionado.';
      return;
    }
    ...
    if (form.invalid) {
      this.markAllTouched(form);
      this.errorMsg = 'Por favor, corrige los errores resaltados.';
      return;
    }

    // Construir payload (normalizando)
  ...
  resetForm(): void {
    // Mantener referencia para no romper ngModel
