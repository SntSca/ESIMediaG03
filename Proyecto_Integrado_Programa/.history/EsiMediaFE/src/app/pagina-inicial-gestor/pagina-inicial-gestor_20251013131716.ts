...
import Swal from 'sweetalert2'; // 👈 añade esto
...
export class PaginaInicialGestor implements OnInit {
  ...
  loading = false;          // puedes usar este como isLoading
  errorMsg = '';
  successMsg = '';
  crearAbierto = false;
  lastSubmitAt = 0;         // 👈 para throttling
  ...
  // ===== Submit (estilo registro con SweetAlert2) =====
  async onSubmit(form: NgForm) {
    ...
    if (this.crossErrorsPresent) {
      ...
      // enfocar primer campo marcado como error
  ...
  resetForm(): void {
    // Mantener referencia (no romper ngModel)
