...
public class ContenidoController {
    ...
    public ResponseEntity<List<Contenido>> listarFiltrados(@RequestBody Map<String, Object> filtros) {
        boolean esVip = false;
Contenido.Tipo tipo = null;

if (filtros != null && filtros.containsKey("tipo")) {
    try {
        final Contenido.Tipo tipoFiltro = Contenido.Tipo.valueOf(filtros.get("tipo").toString().toUpperCase());

        // Filtrado usando la variable final
        ...
        // Obtenemos los contenidos visibles
        List<Contenido> visibles = contenidoService.listarVisibles(esVip);
        if (tipo != null) {
            visibles.removeIf(c -> c.getTipo() != tipo);
        }
