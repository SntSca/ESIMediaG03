...
public class ContenidoService {
    ...
    public Contenido modificarContenido(Contenido contenido) throws Exception {
        if (contenido.getId() == null) throw new IllegalArgumentException(...);
        Contenido existente = contenidoDAO.findById(contenido.getId())
            .orElseThrow(() -> new IllegalArgumentException("No existe contenido..."));

        if (existente.isVisible() != contenido.isVisible()) {
            contenido.setFechaEstado(LocalDateTime.now()); // añade setter para fechaEstado si no tienes
        } else {
            // conservar fecha anterior si no cambia
            contenido.setFechaEstado(existente.getFechaEstado());
        }
