...
public class UserService {
    ...
    public void resetPassword(String token, String newPassword) throws TokenException {
        if (token == null || token.trim().isEmpty()) {
            throw new TokenException("Token no proporcionado");
        }
        ...
        if (user == null || user.getResetPasswordExpires() == null) {
            ...
            throw new TokenException("Token inválido o caducado. Solicita uno nuevo.");
        }

        if (user.getResetPasswordExpires().isBefore(LocalDateTime.now())) {
            logExpiredToken(token, user.getEmail());
            logInvalidToken(token);
            throw new TokenException("Token caducado. Solicita uno nuevo.");
        }
