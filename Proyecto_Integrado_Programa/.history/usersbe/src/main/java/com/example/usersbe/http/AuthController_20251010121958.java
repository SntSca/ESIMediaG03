...
public class AuthController {
    ...
    public ResponseEntity<Object> verify(@RequestBody MfaVerifyRequest request) {
        // Nota: mfaToken se usa como userId en tu AuthService actual
        ...
        // Si el rol exige 3er factor, generamos captcha ASOCIADO al userId
        if (requiresThirdFactor(u)) {
            final CaptchaService.CaptchaPayload cap = captchaService.generate(u.getId());
            return ResponseEntity.ok(LoginResponse.needMfa3(cap.token, cap.imageBase64));
        }

        // Fin de factores → devolvemos user
        return ResponseEntity.ok(LoginResponse.ok(sanitize(u)));
    }
    ...
    public ResponseEntity<Object> verifyCaptcha(@RequestBody CaptchaVerifyRequest req) {
        ...
        // Verificamos: si acierta, consume y devuelve userId asociado
        final String userId = captchaService.verifyAndConsumeReturnUserId(req.getCaptchaToken(), answerNorm);
        if (userId == null) {
            // Falló → rotamos el captcha manteniendo el mismo userId
            ...
            if (cap == null) {
                // No se pudo rotar (expirado/inexistente) → obliga a reemitir MFA3 desde 2FA
                return new ResponseEntity<>(
